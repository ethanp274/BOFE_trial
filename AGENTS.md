...
- The main pipeline runs `R/01_cleaning.R`, `R/02_imputation.R`, `R/03_descriptives.R`, `R/04b_gee.R`, `R/05_cost_effectiveness.R`, `R/06_outputs.R`, and `R/07_manuscript_report.R`. `R/01b_publication_long_dataset.R` is an optional post-cleaning/pre-imputation publication-data export and does not change canonical analysis artifacts.
...
- `run_full_pipeline.ps1` initializes `logs/pipeline_progress.log` from scratch at the start of a full run, then stage scripts append within that run. Manual single-stage script runs append to the existing log for debugging continuity.
...
- **Data source flexibility:** `R/01_cleaning.R` processes raw SPSS files only and writes `data_processed/cleaning_artifact.rds`. `R/02_imputation.R` is the decision point for data source: it reads the cleaning artifact by default (raw data path), or loads the public anonymized CSV directly if `BOFE_DATA_SOURCE="public"`. This enables full replicability using only the published open dataset without access to raw data or re-processing already-clean data. All downstream analyses (imputation, GEE, CEA, tables) produce identical results regardless of data source. See `R/00_methods_config.R` for the `data_source` configuration, `R/public_data_helpers.R` for the public-data loading logic, and `README.md` for usage instructions.
- `R/01b_publication_long_dataset.R` reads `data_processed/cleaning_artifact.rds` and writes `data_processed/bofe_publication_anonymized_long.csv` plus `data_processed/bofe_publication_anonymized_long_codebook.md`; it replaces source patient IDs with random short IDs, omits pharmacy IDs, converts baseline residence to anonymized letter categories, keeps baseline age/gender/BMI/smoking as interpretable categorical labels suitable for reproduced regressions, and writes missing values as literal `NA`.
...
- `R/00_methods_config.R` is the central source of truth for method choices and rationale notes, including outcome thresholds, imputation rules, effectiveness model family/covariates, CEA cost family, EQ-5D tariffs, intervention cost, WTP threshold, bootstrap counts, data source selection, and sensitivity settings.
...
- EQ-5D-5L item responses are valid only at levels 1-5; invalid raw values such as `0` are recoded to `NA` before EQindex scoring and before the CEA imputation branch receives raw EQ-5D item columns.
- `R/02_imputation.R` builds one canonical wide imputation source, derives separate primary-effectiveness and CEA MICE frames, runs 20 arm-split imputations per branch, and writes `data_processed/imputation_artifact.rds` as the canonical imputation artifact. A secondary-effectiveness adherence branch exists behind a disabled config flag but is not part of the clean main pipeline. It also writes readable predictor-selection and `mice::quickpred()` comparison audits in `audit/`. Sensitivity imputation variants live inside `results/sensitivity_analyses_artifact.rds` when `R/08_sensitivity_analyses.R` is run.
...
- `R/03_descriptives.R` writes `results/descriptives_artifact.rds`, bundling the full pre-imputation baseline table (including standardized differences and explicit category/missing rows for multi-level baseline covariates), disease-aware missingness summary, cost summary, and resource-use summary.
...
- `R/08_sensitivity_analyses.R` contains the configured secondary effectiveness and CEA sensitivity analyses, including missing-data-handling GEE comparisons, 12-month model-specification comparisons, alternate imputation variants, intervention-cost sweeps, and an EQ-5D tariff sensitivity that now re-estimates QALYs while reusing the synchronized main-analysis cost bootstrap.
...
- `R/11_control_transition_sankey.R` reads the anonymized publication CSV and writes separate intervention/control Sankey-style PNGs plus aggregate and patient-level CSVs for observed baseline-to-12-month disease-control transitions.
...
- The primary effectiveness MICE frame is deliberately parsimonious: core baseline covariates (`condition`, `gender`, categorical `age`, `BMI`, `smoking`, `ihd`), `controlled_*`, and `EQindex_*`; costs, raw EQ-5D items, adherence variables, and sparse resource-use auxiliaries are excluded
...
## Cost data
- Imputed in a separate CEA branch using core baseline covariates (`condition`, `gender`, categorical `age`, `BMI`, `smoking`, `ihd`), `controlled_*`, raw `EQ5D5L.*` item columns, and canonical half-year cost summaries
...
- 2026-06-01: Reworked the `basic` MICE sensitivity to match the legacy predictor matrix exactly: baseline age/sex, baseline control, baseline EQindex, and lagged control predictors only. This branch was retired on 2026-06-03 after the main MICE predictor policy was simplified.
...
- 2026-06-03: Further simplified the primary MICE baseline predictor set to condition, gender, categorical age, BMI, and smoking, removing weak or redundant auxiliaries including selection and num_meds. Re-ran `R/02_imputation.R`, `R/04b_gee.R`, `R/06_outputs.R`, and `R/07_manuscript_report.R`; the effectiveness MICE frame now has 17 variables, 8 imputed targets, and 130 predictor links, the CEA MICE frame has 47 variables, 34 imputed targets, and 1165 predictor links, and the adjusted 12-month GEE OR is 1.399 (95% CI 1.000 to 1.956). Retired the separate `basic` MICE sensitivity branch so imputation sensitivity now compares main MICE, simple/naive within-arm imputation, and complete-case analysis. CEA modelling was not intentionally refreshed in the age-coding turn; rerun the full pipeline when edits settle so CEA outputs are unquestionably synchronized.
- 2026-06-03: Added baseline IHD to the configured MICE predictor set after in-memory sensitivity testing showed it was complete, clinically defensible, and did not weaken the adjusted 12-month GEE contrast when tested alone.
- 2026-06-03: Fixed EQ-5D-5L item cleaning so impossible item values outside 1-5, especially all-zero follow-up profiles, are recoded to `NA` before EQindex scoring and imputation. Added validation checks so cleaning/imputation smoke tests fail if raw EQ-5D items contain values outside 1-5.
- 2026-06-03: Added secondary medication-adherence and last-missed-dose analyses. `D5.9 == 3` ("doesn't know") and `D5.10 == 5` ("can't remember") are now recoded to missing before imputation. Added a separate `secondary_effectiveness_mids` branch so adherence targets are imputed with the same time-aware predictor classes as `controlled_*` without perturbing the primary disease-control MICE branch. `R/04b_gee.R` now reports secondary GEE models and `R/04_models.R` includes matching GLMM sensitivity paths. Re-ran `R/02_imputation.R` and `R/04b_gee.R`; adjusted 12-month primary GEE OR remains 1.414 (95% CI 1.010 to 1.979), adjusted medication-adherence OR is 1.055 (95% CI 0.715 to 1.557), and adjusted non-recent-missed-dose OR is 1.422 (95% CI 1.015 to 1.992). `R/06_outputs.R` and `R/07_manuscript_report.R` were updated but could not be rerun in this turn because the workspace approval spend cap blocked further Rscript execution.
- 2026-06-03: Deprecated the medication-adherence and last-missed-dose secondary analysis path after review suggested the current `D5.9`/`D5.10` coding was not methodologically meaningful enough for the clean manuscript pipeline. `R/00_methods_config.R` now sets `effectiveness.secondary_outcomes.enabled = FALSE`; `R/02_imputation.R` no longer builds `secondary_effectiveness_mids` by default; `R/04b_gee.R`, `R/04_models.R`, `R/06_outputs.R`, and `R/07_manuscript_report.R` skip secondary adherence modelling/exports unless the flag is deliberately re-enabled. The clean rerun should focus on primary GEE effectiveness and CEA.
- 2026-06-18: Added and ran `R/09_imputation_predictor_matrix_audit.R` to audit the primary-effectiveness MICE predictor matrix. The audit confirmed zero future-time links in the current analytic matrix, rejected future-time quickpred suggestions, flagged weak non-anchor baseline auxiliary links for removal sensitivity, and wrote predictor-matrix audit CSV/Markdown outputs under `audit/`.
- 2026-06-18: Added and ran `R/10_imputation_matrix_sensitivity.R` to rerun primary-effectiveness MICE/GEE under the current analytic matrix, no-utility-index auxiliaries, no same-visit auxiliaries, history-only controlled outcomes, and sanitized quickpred variants. Adjusted 12-month GEE ORs ranged from 1.396 to 1.443, showing directional stability with two variants near the 0.05 threshold.
- 2026-06-18: Added `audit/imputation_matrix_sensitivity_manuscript_note.md` so the predictor-matrix sensitivity analysis can be described later without changing the primary imputation method or manuscript-facing main results.
- 2026-06-18: Added `audit/master_robustness_and_sensitivity_note.md`, a single reviewer-facing robustness dossier covering adjusted/unadjusted GEE, GEE versus mixed-effects sensitivity, imputation predictor-matrix stress tests, full/simple/complete-case missing-data checks, CEA imputation variants, cost-model assumptions, tariff sensitivity, intervention-cost sweeps, nested MI bootstrap uncertainty, and deprecated secondary adherence analyses.
- 2026-06-18: Added a manuscript text-editing handoff section to `README.md`, compressing the current pipeline status, primary numeric results, robustness/sensitivity narrative, stale-output cautions, and refresh checklist for future agents working on manuscript wording or reviewer responses.
- 2026-06-18: Ran `R/04_models.R` and `R/08_sensitivity_analyses.R` without rerunning the main GEE, main CEA, or manuscript-output stages. This refreshed the mixed-effects sensitivity artifact plus effectiveness, CEA imputation, intervention-cost, and UK EQ-5D tariff sensitivity CSVs; updated README/master robustness notes with the refreshed sensitivity results.
...
- 2026-06-24: Added absolute standardized differences to the Table 1 baseline characteristic comparison in `R/03_descriptives.R` for both continuous and binary variables, reran the descriptives stage, and refreshed `results/table1_complete_cases_characteristics.csv` so the baseline balance table now reports `Std_Diff` alongside the existing group summaries, missingness counts, and p-values.
- 2026-06-24: Expanded `R/03_descriptives.R` so Table 1 now includes multi-level categorical baseline blocks for condition, age category, ethnicity, education, selection source, BMI category, and smoking status, with one row per category plus an explicit `Missing` row for each variable. Re-ran the descriptives stage and refreshed `results/table1_complete_cases_characteristics.csv`.
- 2026-06-24: Switched the Table 1 baseline characteristic comparison to use the full pre-imputation cohort (`all_cases`, N=835) rather than the analyzed complete-case subset, because missingness is now represented directly in the table. `R/03_descriptives.R` now writes the refreshed table to `results/table1_all_cases_characteristics.csv` and retains `results/table1_complete_cases_characteristics.csv` as a best-effort legacy alias when the file is not locked by another process.
- 2026-06-23: Reviewed the rerun sensitivity outputs after the bootstrap-policy update and refreshed `audit/master_robustness_and_sensitivity_note.md` plus `README.md` so the manuscript handoff note matches the current artifacts on disk. The main CEA artifacts are now synchronized at 5000 bootstrap draws, while complete-case, simple-imputation, and UK tariff CEA sensitivity branches report refreshed 1500-draw results.
- 2026-06-23: Extended the longitudinal effectiveness helper so the repeated-measures data retain baseline `condition`, `BMI`, `smoking`, and `ihd`, then added dedicated 12-month model-specification and missing-data-handling comparison outputs. New outputs include `results/model_specification_12mo_comparison.csv` and `results/missing_data_handling_gee_12mo.csv`, and `results/sensitivity_analyses_artifact.rds` was synced to carry those new comparison tables. Current model-specification comparison shows OR 1.414 for the main adjusted GEE, 1.386 for the expanded-covariate adjusted GEE, 1.392 for the adjusted 12-month-only GLM, 1.734 for the adjusted patient-random-effect GLMM, and 1.687 for the adjusted patient-plus-pharmacy GLMM.
- 2026-06-23: Reworked the UK/England EQ-5D tariff sensitivity so it now reuses the synchronized main-analysis incremental-cost bootstrap and re-estimates only the QALY side under the alternate tariff. `R/05_cost_effectiveness_helpers.R` now contains a QALY-only nested MI bootstrap helper plus a tariff-sensitivity wrapper that recenters the reused main cost draws to the main pooled incremental-cost estimate. Refreshed `results/cea_tariff_sensitivity_summary.csv` now keeps incremental cost at -116.75 EUR by design, with alternate-tariff incremental QALY 0.0195 and probability cost-effective 0.835.

Next steps:
1. Compare `results/model_specification_12mo_comparison.csv`, `results/missing_data_handling_gee_12mo.csv`, `results/cea_summary.csv`, `results/manuscript_results_summary.csv`, and `audit/master_robustness_and_sensitivity_note.md` against the manuscript draft before changing reported text.
2. Use `Rscript R/run_smoke_tests.R` before and after expensive stages to catch shape or artifact regressions quickly.
3. If upstream artifacts change again, keep the main CEA at 5000 bootstrap draws and expect refreshed complete-case, simple-imputation, and UK tariff sensitivity branches to report `n_boot = 1500` unless deliberately overridden.
4. If a full end-to-end rerun is needed, run `.\run_full_pipeline.ps1` from the project root.

## Handoff summary for next agent

Summary: The modular pipeline now has cleaning derivations, descriptives, follow-up-only GEE/mixed-effects models, a synchronized complex-MICE main CEA branch, and a separate `R/08_sensitivity_analyses.R` runner for missing-data, model-specification, and CEA sensitivity scenarios. The former monolithic `R/utils.R` has been split into focused helper modules, `R/00_contracts.R` defines/enforces data-shape contracts, and `R/00_methods_config.R` is the source of truth for method choices. Each active stage writes one canonical RDS artifact, and downstream scripts read those artifacts instead of legacy per-table CSV/RDS sidecars. The nested MI CEA bootstrap now resamples prepared patient-level CEA frames rather than rebuilding them inside each draw. Current manuscript-facing economic outputs are synchronized at 5000 bootstrap draws, while refreshed CEA sensitivity reruns default to 1500.
...
1. Use `results/model_specification_12mo_comparison.csv` for manuscript discussion of outcome-model sensitivity and `results/missing_data_handling_gee_12mo.csv` for manuscript discussion of missing-data sensitivity.
2. Keep complete-case and simple-imputation effectiveness checks grouped with predictor-matrix variants as missing-data-handling sensitivity, not as alternative primary model specifications.
3. If upstream artifacts change again, rerun `source("R/08_sensitivity_analyses.R")` after the main artifacts are fresh, then inspect `results/sensitivity_analyses_artifact.rds`.
4. If a full end-to-end refresh is needed, run `.\run_full_pipeline.ps1`.
...
## Additional log entry (2026-06-22)

- 2026-06-22: **Added public dataset replicability path** — Created `R/public_data_helpers.R` with functions to load and reshape the public anonymized CSV for full replicability without raw data access. Updated `R/00_methods_config.R` to add `data_source` configuration (choices: "raw" or "public") with environment override support (`BOFE_DATA_SOURCE`). Modified `R/01_cleaning.R` to branch on data source: reads either raw SPSS files or the public CSV, with identical downstream artifact output. Updated `R/utils.R` to source the new public data helper. Updated README.md with a "Quick Start" section for reproducing with the public dataset. Updated AGENTS.md to document the data source flexibility feature. All downstream analyses (imputation, GEE, CEA, tables) now work identically regardless of data source, enabling independent researchers to verify results using only the published open dataset at https://dx.doi.org/10.5287/ora-rjkzjgg4k.
- 2026-06-22: **Refined architecture to avoid re-processing clean data** — Moved the public data-source decision from `R/01_cleaning.R` (which now processes raw SPSS files only) to `R/02_imputation.R` (the true decision point). This eliminates unnecessary duplication: the public dataset is already post-cleaning, so it should not re-apply cleaning logic, structural-zero rules, or outcome derivations. `R/02_imputation.R` now checks `BOFE_DATA_SOURCE` and either reads the canonical `cleaning_artifact.rds` (raw path) or loads the public CSV directly (public path), avoiding redundant processing. Both paths converge to the same wide imputation frame. Updated README.md to clarify that users with only the public dataset can skip `R/01_cleaning.R` entirely and start from `R/02_imputation.R`. Updated AGENTS.md and pipeline status documentation accordingly. This is cleaner conceptually and architecturally sound: raw data goes through cleaning, pre-clean data bypasses it.
