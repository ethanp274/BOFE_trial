...
- The effectiveness MICE frame is deliberately parsimonious: core baseline covariates (`condition`, `gender`, categorical `age`, `BMI`, `smoking`, `ihd`), `controlled_*`, and `EQindex_*`; costs, raw EQ-5D items, adherence variables, and sparse resource-use auxiliaries are excluded
