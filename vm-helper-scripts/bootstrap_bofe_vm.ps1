...
if ($BootstrapWorkers -gt 0) {
  [Environment]::SetEnvironmentVariable("BOFE_BOOTSTRAP_WORKERS", $BootstrapWorkers.ToString(), "Machine")
  $env:BOFE_BOOTSTRAP_WORKERS = $BootstrapWorkers.ToString()
  Write-Log ("Set BOFE_BOOTSTRAP_WORKERS={0}" -f $BootstrapWorkers)
}
...
if ($RunPipeline) {
  ...
  Write-Log ("Running the BOFE pipeline from {0}" -f $RepoPath)
  Push-Location $RepoPath
  try {
    $pipelineScript = Join-Path $RepoPath "run_full_pipeline.ps1"
    & $pipelineScript
    if ($LASTEXITCODE -ne 0) {
      throw "run_full_pipeline.ps1 failed with exit code $LASTEXITCODE."
    }
  } finally {
    Pop-Location
  }
